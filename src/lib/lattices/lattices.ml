...
module Uniform_product_lattice
    (L : Sig.Lattice) (N : sig
      val n : int
    end) : Sig.Lattice = struct
...
module Reverse_powerset_lattice (D : sig
  include Sig.Element
...
struct
  ...
  let lub x y =
    let open Map.Infix in
    Map.keys x
    |> Enum.fold (fun acc k -> acc <-- (k, L.lub (x --> k) (y --> k))) y
...
module Lattice_with_bottom (L : Sig.Lattice) = struct
  ...
  let extract = function Bottom -> raise Bottom_exception | Some x -> x

  let to_string = function Bottom -> "_" | Some x -> L.to_string x
end

module Lattice_with_top (L : Sig.Lattice) = struct
...
module Lattice_with_bottom_top (L : Sig.Lattice) = struct
  type property = Top | Bottom | Element of L.property
  ...
  let equal x y =
    ...
    | Element _, Bottom ->
        false
    | Element a, Element b -> L.equal a b

  let lub x y =
    match (x, y) with
    | Element a, Element b -> Element (L.lub a b)
    | Bottom, a | a, Bottom -> a
    | Top, _ | _, Top -> Top
  ...
  let to_string = function
    | Top -> "top"
    | Bottom -> "bottom"
    | Element x -> L.to_string x
end

module Sequence_lattice (L : Sig.Lattice) = struct
  type property = L.property list
